...
import { BACKLOG_PROJECT_ID, DEFAULT_PROJECT_ID } from "@/config/defaultProjects";
...
export function KanbanHeader({
  ...
  const { selectableProjects, getLabel } = useProjects();
  const [titleInput, setTitleInput] = useState("");
  const firstBoardProject = selectableProjects[0]?.id ?? DEFAULT_PROJECT_ID;
  const [brainProject, setBrainProject] = useState(firstBoardProject);
  const projectForNewTask = fixedProjectId
    ? fixedProjectId
    : projectFilter !== "all" && !isBacklogProjectId(projectFilter)
      ? projectFilter
      : brainProject;
  ...
  function submit(e?: FormEvent) {
    ...
    addTask(
      trimmed,
      fixedProjectId ?? projectForNewTask ?? firstBoardProject,
    );
    setTitleInput("");
  }
        ...
        {showBrainDump ? (
          ...
          >
            {fixedProjectId ? (
              <div
                className={cn(
                  "flex shrink-0 items-center rounded-l-2xl border-r px-3 py-3 text-[10px] font-bold uppercase tracking-widest sm:px-4 sm:py-3.5 sm:text-xs md:py-4",
                  t(
                    isDark,
            ...
            <input
              ...
              placeholder={
                fixedProjectId === BACKLOG_PROJECT_ID
                  ? "Nový nápad do backlogu…"
                  : "Nová úloha…"
              }
              className={cn(
