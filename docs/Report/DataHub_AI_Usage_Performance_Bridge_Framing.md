...
Performance vrstva je **samostatná, jasne definovaná** časť požiadavky — **nie zlučovanie usage a performance vrstvy do jedného všeobecného „AI impact“ rámca**.
...
Ak niektorý porovnávací rez nie je v prvom kole dostupný, **neznamená to stopnutie datasetu** — zmysel má stále článkový riadok s usage + dostupnými performance poľami a **explicitne označená medzera** v komparačnej dimenzii.
...
Skrátený **data request** v jazyku analytiky: grain, join kľúče, **eligible sample / CAP definícia per feature** (inclusion / exclusion v analytickom zápise), zdroje pre usage vrstvu, zdroje pre performance signály, definícia **komparačnej bázy** (used vs not-used within eligible, prípadné časové okná), **jasný popis comparison logic / baseline rez** pre daný výstup (pozri §6), povinné vs voliteľné polia, periodicita exportu. Detailné KPI reporting UX zostáva v zadaní [**CMSB-1810**](./task+kontext+gpt.md); **tento framing** zjednocuje očakávania pri implementácii.
