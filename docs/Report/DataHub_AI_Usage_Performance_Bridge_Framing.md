...
Až po dodaní Pent podkladov (sub-task pod **DHUB-3138**) môže nasledovať samostatný technický request v jazyku analytiky podľa princípov tohto dokumentu (grain, join kľúče, CAP per feature, recurring výstup, dokumentácia medzier). **Do Pent sub-tasku sa tento krok neprepája.**
...
**Copy-paste pre Jiru (Penta):** finálne znenie **sub-tasku** pod DHUB-3138 — **[`DHUB-3138_Penta_Data_Delivery.md`](./DHUB-3138_Penta_Data_Delivery.md)**. Rozšírený návrh po Pent („upratovanie“): **[`DataHub_AI_Reporting_Phase2_PostPenta.md`](./DataHub_AI_Reporting_Phase2_PostPenta.md)** — interné, nie pre DH pri Pent. **Tento framing súbor ani Phase 2 súbor do komunikácie s DH pri Pent neposielať.**
